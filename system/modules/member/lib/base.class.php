...
class HEC
{
	...
	public function login($username, $token){
        ...
        if ($login_user['username'] == null){
        	$decode = 0;
        	$sql="INSERT INTO `@#_member`(username,nickname,money,token,img,emailcode,mobilecode,yaoqing,time) VALUES('$username','$displayName','$availableScores','$token','photo/member.jpg','1','1','$decode','$time')";
        	$this->db->Query($sql);
        	$login_user=$this->db->GetOne("select * from `@#_member` where `username` = '$username' ORDER BY `uid` DESC LIMIT 1");
        	...
        	$sql="UPDATE `@#_member` SET money='$availableScores', nickname='$displayName', token='$token', time='$time' WHERE username='$username'";
        	$this->db->Query($sql);
        	// echo $login_user['uid'] . '<br>';
        	// echo "update";
        }
